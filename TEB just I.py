...
dark_image           = False
height_delimeters = [.45,0.65]
I_0 = 1000*intensity_in_image(par_pol_path, height_delimeters, show_patch)
time_of_recording = 22 #in seconds s
#PARAMETERS for find_drop: (see function description)
threshold = 0.7 #threshold in percentage to find drop higher for noisy data
...
for file_path in file_paths:
    ...
    t = frames/fps
    OR = I # if you want to keep I just comment this
    ...
    fit_len = int(0.2*(len(OR) - begin_drop ))  #lenght of the fit
    b_0 = 10
    if dark_image == False:
        c_0 = np.mean(OR[len(OR)-3:len(OR)] )  #OR[len(OR)-1] OR[begin_drop+fit_len]  max(OR)
    if dark_image == True:
        ...
        plt.axhline(I_dark)
        plt.show()
    a_0 = np.mean(OR[begin_drop-10:begin_drop] ) - c_0
    ...
    [d, a, err_d, err_a] = perform_st_exp_fit(t,OR,begin_drop+1,fit_len,st_exponential,visualize_fit,fps,title,savepath) # d and a are D and alpha
    D.append(d)
    alpha.append(a)
    err_D.append(err_d)
...
I_EG =   (maxEG-minEG)* np.exp( -(6*0.139* np.arange(len(frames)-begin_drop)/fps)**0.5347) + minEG 



plt.plot(I[begin_drop:len(frames)], label = 'I')
plt.plot(I_EG, label = 'EG')
plt.legend()
plt.show()
# %%
