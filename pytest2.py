...
#     """
#     GIVEN: random values are given to the images if the multipage tif
...
#     assert I == intensity_in_multipage_image('multipage_image.tif',[0.35,0.66])

@given(st.lists(st.integers(min_value=0, max_value=200), min_size=1, max_size=100))
def test_intensityi_in_multiple_image(I):
    """
    GIVEN: random values are given to the images if the multipage tif
    WHEN: measure the values of the intensity of the images
    THEN: the measured list has to be equal to the random one generated
    ...
    # Run the intensity measurement function
    measured_intensities = intensity_in_multipage_image('multipage_image.tif', [0.35, 0.66])
    
    # Compare the measured intensities to the original intensities
    assert I == measured_intensities
...
def test_exp_fit(b,a):
    ...
    GIVEN: two random integers b in (0,100) and a in (0.1,1)
    WHEN: fitting the stretched exponential created with the two numbers 
    THEN: the fit must give results within the threshold error
    """
    def st_exp(x,b,a):
        return np.exp(-(b*x)**a)
